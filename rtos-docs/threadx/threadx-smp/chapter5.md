...
> [!IMPORTANT]
> O componente da função de inicialização do controlador é normalmente chamado a partir da função **tx_application_define** ou de um fio de inicialização.
...
Esta área de função é responsável por fornecer o estado de funcionamento e as estatísticas associadas à operação do condutor. As informações geridas por esta área de função normalmente incluem o seguinte: 
...
Esta área funcional é opcional. Só é necessário desligar o condutor e/ou o dispositivo de hardware físico. Após a sua demissão, o condutor não deve ser chamado novamente até que seja re-inicializado. 
...
A função de inicialização também é responsável pela instalação dos manipuladores de vetores de interrupção de baixo nível para notificações de entrada e saída. Tal como outras rotinas de serviço de interrupção da ThreadX SMP, o manipulador de baixo nível deve ligar para ***_tx_thread_context_save** _ antes de ligar para o isr do controlador simples. Após a retorna do controlador ISR, o manipulador de baixo nível deve ligar para _*_ _tx_thread_context_restore_**.
...
Consulte a figura 9 na página 306 para obter o código fonte de inicialização do controlador simples.
...
> [!IMPORTANT]
> Apenas os fios podem chamar a função **tx_sdriver_input.**
...
> [!IMPORTANT]
> Apenas os fios podem chamar a função **tx_sdriver_output.**
...
Este exemplo simples do controlador do dispositivo ilustra a ideia básica de um controlador de dispositivoS ThreadX SMP. No entanto, uma vez que o simples controlador do dispositivo não aborda o buffering de dados ou quaisquer problemas de sobrecarga, não representa totalmente os controladores SMP da ThreadX no mundo real. A secção seguinte descreve alguns dos problemas mais avançados associados aos condutores do dispositivo.
...
Cada tampão de byte circular é composto por uma área de memória byte (tipicamente uma matriz de UCHARs), um ponteiro de leitura e um ponteiro de escrita. Um tampão é considerado vazio quando o ponteiro de leitura e os ponteiros de escrita referenciam o mesmo local de memória no tampão. A inicialização do condutor define tanto os ponteiros de leitura como os ponteiros tampão para o endereço inicial do tampão.
...
> [!IMPORTANT]
> Para um funcionamento fiável, pode ser necessário bloquear as interrupções ao manipular os indicadores de leitura e escrever tanto os tampões circulares de entrada como os tampões circulares de saída.

### <a name="circular-output-buffer"></a>Tampão de saída circular 
O tampão de saída é utilizado para segurar caracteres que chegaram para a saída antes do dispositivo de hardware terminar de enviar o byte anterior. O processamento do tampão de saída é semelhante ao processamento do tampão de entrada, exceto que o processamento de interrupção completa de transmissão manipula o ponteiro de leitura de saída, enquanto o pedido de saída da aplicação utiliza o ponteiro de escrita de saída. Caso contrário, o processamento do tampão de saída é o mesmo. A figura 13shows a lógica para o tampão de saída circular.
...
O tamanho e localização dos amortecedores de I/S é determinado pela aplicação e/ou software do condutor. Normalmente, os tampão são fixos em tamanho e geridos dentro de um conjunto de memória de blocos ThreadX SMP. A Figura 14 atribui um tampão de E/S típico e um conjunto de memória de blocos ThreadX SMP que gere a sua alocação.
...
O TX_IO_BUFFER da tipa é composto por dois ponteiros. O ponteiro ***tx_next_packet** _ é utilizado para ligar vários pacotes na lista de entradas ou saídas. O ponteiro _ *_tx_next_buffer_** é utilizado para ligar tampãos que compõem um pacote individual de dados do dispositivo. Ambos os ponteiros são definidos para NU QUANDO o tampão é atribuído a partir da piscina. Além disso, alguns dispositivos podem necessitar de outro campo para indicar quanto da área tampão realmente contém dados.
...
Em algumas aplicações, a frequência de interrupção do dispositivo pode proibir a escrita do ISR em C ou interagir com o ThreadX SMP em cada interrupção. Por exemplo, se for preciso 25us para salvar e restaurar o contexto interrompido, não seria aconselhável realizar um contexto completo, salvo se a frequência de interrupção fosse de 50us. Nesses casos, uma pequena linguagem de montagem ISR é usada para manusear a maioria das interrupções do dispositivo. Este ISR lowoverhead só interagiria com o ThreadX SMP quando necessário.

Uma discussão semelhante pode ser encontrada na discussão de gestão interrompida no final do capítulo 3.

> [!NOTE]
> Se o bloqueio de interrupção for utilizado para proteger as secções críticas do código do controlador do controlador do código ISR, o código do controlador de nível de fio deve ser sempre executado no mesmo núcleo em que o ISR é processado. Caso contrário, devem ser utilizados primitivos Internos ThreadX SMP como TX_DISABLE e TX_RESTORE que também tenham proteção inter-core incorporada.
