---
title: Apêndice D - Azure RTOS NetX Duo Avançado DHCPv6 exemplo
description: Este capítulo contém um exemplo avançado do servidor NETX Duo DHCPv6
author: philmea
ms.author: philmea
ms.date: 06/08/2020
ms.topic: article
...
ms.locfileid: "104826048"
---
# <a name="appendix-d----azure-rtos-netx-duo-advanced-dhcpv6-server-example"></a><span data-ttu-id="d53ac-103">Apêndice D - Azure RTOS NetX Duo Avançado DHCPv6 exemplo</span><span class="sxs-lookup"><span data-stu-id="d53ac-103">Appendix D -  Azure RTOS NetX Duo Advanced DHCPv6 server example</span></span>

<span data-ttu-id="d53ac-104">Este é um servidor DHCPv6 avançado que demonstra a poupança e recuperação da tabela de locação de endereços IP do Servidor e tabelas de registo do Cliente a partir de memória não volátil, conforme exigido pelo RFC 3315.</span><span class="sxs-lookup"><span data-stu-id="d53ac-104">This is an advanced DHCPv6 Server which demonstrates saving and retrieving the Server’s IP address lease table and Client record tables from non volatile memory, as required by the RFC 3315.</span></span>

<span data-ttu-id="d53ac-105">Neste exemplo, o ficheiro incluído *nxd_dhcpv6_server.h* é trazido na linha 7.</span><span class="sxs-lookup"><span data-stu-id="d53ac-105">In this example, the include file *nxd_dhcpv6_server.h* is brought in at line 7.</span></span> <span data-ttu-id="d53ac-106">Em seguida, o fio de aplicação do Servidor NetX Duo DHCPv6 é criado na linha 81 no código de exemplo abaixo.</span><span class="sxs-lookup"><span data-stu-id="d53ac-106">Next, the NetX Duo DHCPv6 Server application thread is created at line 81 in the example code below.</span></span> <span data-ttu-id="d53ac-107">Note que o bloco de controlo DHCPv6 "*dhcp_server_0*" foi definido como uma variável global na linha 19 anteriormente.</span><span class="sxs-lookup"><span data-stu-id="d53ac-107">Note that the DHCPv6 control block “*dhcp_server_0*” was defined as a global variable at line 19 previously.</span></span>

<span data-ttu-id="d53ac-108">Antes de criar a instância do Servidor NetX Duo DHCPv6, a demo cria um pacote de conjuntos para o envio de mensagens DHCPv6 na linha 84, cria uma interface de fio IP na linha 102 e permite uDP na NetX Duo na linha 116.</span><span class="sxs-lookup"><span data-stu-id="d53ac-108">Before creating the NetX Duo DHCPv6 Server instance, the demo creates packet pool for sending DHCPv6 messages in line 84, creates an IP thread interface in line 102, and enables UDP in NetX Duo in line 116.</span></span>

<span data-ttu-id="d53ac-109">A criação bem sucedida do NetX Duo DHCPv6 Server na linha 136 inclui a função de callbacks opcional descrita no Capítulo 1.It permite que o IPv6 e o ICMPv6 necessários para o NetX Duo processe as operações IPv6 e DHCPv6 na linha 162-163.</span><span class="sxs-lookup"><span data-stu-id="d53ac-109">The successful creation of NetX Duo DHCPv6 Server in line 136 includes the the two optional callbacks function described in Chapter 1.It enables IPv6 and ICMPv6 necessary for NetX Duo to process IPv6 and DHCPv6 operations in line 162-163.</span></span> <span data-ttu-id="d53ac-110">Antes de a linha do Servidor DHCPv6 estar pronta a ser executada, o servidor DHCPv6 deve validar o seu endereço IPv6 (167-180) e definir a sua interface DHCPv6 nas linhas 208-209.</span><span class="sxs-lookup"><span data-stu-id="d53ac-110">Before the DHCPv6 Server thread is ready to run, the DHCPv6 server must validate its IPv6 address(167-180),and define its DHCPv6 interface in lines 208-209.</span></span> <span data-ttu-id="d53ac-111">O serviço *nx_dhcpv6_set_server_duid* é chamado para criar o Servidor se nenhum DUID do Servidor tiver sido previamente criado na linha 266.</span><span class="sxs-lookup"><span data-stu-id="d53ac-111">The *nx_dhcpv6_set_server_duid* service is called to create the Server if no Server DUID has been previously created in line 266.</span></span> <span data-ttu-id="d53ac-112">O Servidor configura um intervalo de endereços IP para criar uma lista de endereços atribuíveis.</span><span class="sxs-lookup"><span data-stu-id="d53ac-112">The Server sets up an IP address range for creating a list of assignable addresses.</span></span> <span data-ttu-id="d53ac-113">Se os dados forem guardados de uma sessão anterior, obtém registos de Clientes e dados de locação IPv6 da memória nas linhas 283-318.</span><span class="sxs-lookup"><span data-stu-id="d53ac-113">If data is saved from a previous session, it retrieves Client records and IPv6 lease data from memory in lines 283-318.</span></span> <span data-ttu-id="d53ac-114">Também cria o seu Server DUID, ou se um foi previamente criado, recupera os dados DUID do armazenamento especificado pelo utilizador.</span><span class="sxs-lookup"><span data-stu-id="d53ac-114">It also creates its Server DUID, or if one was previously created, retrieves the DUID data from user specified storage.</span></span> <span data-ttu-id="d53ac-115">Isto é necessário para reproduzir um SERVIDOR DUID consistente através de reboots.</span><span class="sxs-lookup"><span data-stu-id="d53ac-115">This is necessary to reproduce a consistent Server DUID across reboots.</span></span> <span data-ttu-id="d53ac-116">Opcionalmente, a aplicação do anfitrião define um servidor DNS para clientes que solicitam a configuração do servidor DNS.</span><span class="sxs-lookup"><span data-stu-id="d53ac-116">Optionally the host application defines a DNS server for Clients requesting DNS server configuration.</span></span>

<span data-ttu-id="d53ac-117">Em seguida, o anfitrião inicia o DHCPv6Server na linha 329.</span><span class="sxs-lookup"><span data-stu-id="d53ac-117">Next, the host starts the DHCPv6Server in line 329.</span></span> <span data-ttu-id="d53ac-118">Isto cria a tomada UPD do servidor DHCPv6 e ativa os temporizadores do Servidor NetX Duo DHCPv6.</span><span class="sxs-lookup"><span data-stu-id="d53ac-118">This creates the DHCPv6 Server UPD socket and activates NetX Duo DHCPv6 Server timers.</span></span> <span data-ttu-id="d53ac-119">Em seguida, o Servidor aguarda para receber os pedidos do Cliente.</span><span class="sxs-lookup"><span data-stu-id="d53ac-119">Then the Server waits to receive Client requests.</span></span> <span data-ttu-id="d53ac-120">Embora possa servir muitos Clientes, só pode processar um único pedido de Cliente de cada vez.</span><span class="sxs-lookup"><span data-stu-id="d53ac-120">While it can service many Clients it can only process a single Client request at a time.</span></span>

<span data-ttu-id="d53ac-121">O restante do exemplo contém implementações de anfitriões para guardar e recuperar as tabelas do Servidor do seu pool de endereços IPv6 atribuível e registos do Cliente de e para memória não volátil, respectivamente.</span><span class="sxs-lookup"><span data-stu-id="d53ac-121">The remainder of the example contains host implementations for saving and retrieving Server tables of its assignable IPv6 address pool and Client records to and from non volatile memory respectively.</span></span> <span data-ttu-id="d53ac-122">Também contém um option handler para opções solicitadas pelos Clientes DHCPv6 que não são suportados diretamente pelo NetX Duo DHCPv6 Server (apenas a opção de servidor DNS está suportada atualmente).</span><span class="sxs-lookup"><span data-stu-id="d53ac-122">It also contains an option handler for options requested by DHCPv6 Clients that are not supported directly by the NetX Duo DHCPv6 Server (only the DNS server option is currently supported).</span></span> <span data-ttu-id="d53ac-123">Por último, existe um código para demonstrar como guardar e recuperar "tempo não volátil" pelo qual o Servidor regista a expiração do contrato de arrendamento IP atribuído.</span><span class="sxs-lookup"><span data-stu-id="d53ac-123">Lastly there is a code for demonstrating how to save and retrieve ‘non volatile time’ by which the Server keeps track of assigned IP lease expiration.</span></span>
...
<span data-ttu-id="d53ac-124">**Figura 6. Aplicação avançada do servidor NetX Duo DHCPv6**</span><span class="sxs-lookup"><span data-stu-id="d53ac-124">**Figure 6. Advanced NetX Duo DHCPv6 Server Application**</span></span>