...
ms.locfileid: "116783684"
---
# <a name="appendix-d----azure-rtos-netx-duo-advanced-dhcpv6-server-example"></a>Apêndice D - Azure RTOS NetX Duo Avançado DHCPv6 exemplo

Este é um servidor DHCPv6 avançado que demonstra a poupança e recuperação da tabela de locação de endereços IP do Servidor e tabelas de registo do Cliente a partir de memória não volátil, conforme exigido pelo RFC 3315.

Neste exemplo, o ficheiro incluído *nxd_dhcpv6_server.h* é trazido na linha 7. Em seguida, o fio de aplicação do Servidor NetX Duo DHCPv6 é criado na linha 81 no código de exemplo abaixo. Note que o bloco de controlo DHCPv6 "*dhcp_server_0*" foi definido como uma variável global na linha 19 anteriormente.

Antes de criar a instância do Servidor NetX Duo DHCPv6, a demo cria um pacote de conjuntos para o envio de mensagens DHCPv6 na linha 84, cria uma interface de fio IP na linha 102 e permite uDP na NetX Duo na linha 116.

A criação bem sucedida do NetX Duo DHCPv6 Server na linha 136 inclui a função de callbacks opcional descrita no Capítulo 1.It permite que o IPv6 e o ICMPv6 necessários para o NetX Duo processe as operações IPv6 e DHCPv6 na linha 162-163. Antes de a linha do Servidor DHCPv6 estar pronta a ser executada, o servidor DHCPv6 deve validar o seu endereço IPv6 (167-180) e definir a sua interface DHCPv6 nas linhas 208-209. O serviço *nx_dhcpv6_set_server_duid* é chamado para criar o Servidor se nenhum DUID do Servidor tiver sido previamente criado na linha 266. O Servidor configura um intervalo de endereços IP para criar uma lista de endereços atribuíveis. Se os dados forem guardados de uma sessão anterior, obtém registos de Clientes e dados de locação IPv6 da memória nas linhas 283-318. Também cria o seu Server DUID, ou se um foi previamente criado, recupera os dados DUID do armazenamento especificado pelo utilizador. Isto é necessário para reproduzir um SERVIDOR DUID consistente através de reboots. Opcionalmente, a aplicação do anfitrião define um servidor DNS para clientes que solicitam a configuração do servidor DNS.

Em seguida, o anfitrião inicia o DHCPv6Server na linha 329. Isto cria a tomada UPD do servidor DHCPv6 e ativa os temporizadores do Servidor NetX Duo DHCPv6. Em seguida, o Servidor aguarda para receber os pedidos do Cliente. Embora possa servir muitos Clientes, só pode processar um único pedido de Cliente de cada vez.

O restante do exemplo contém implementações de anfitriões para guardar e recuperar as tabelas do Servidor do seu pool de endereços IPv6 atribuível e registos do Cliente de e para memória não volátil, respectivamente. Também contém um option handler para opções solicitadas pelos Clientes DHCPv6 que não são suportados diretamente pelo NetX Duo DHCPv6 Server (apenas a opção de servidor DNS está suportada atualmente). Por último, existe um código para demonstrar como guardar e recuperar "tempo não volátil" pelo qual o Servidor regista a expiração do contrato de arrendamento IP atribuído.
...
**Figura 6. Aplicação avançada do servidor NetX Duo DHCPv6**