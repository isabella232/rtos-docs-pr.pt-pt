...
ms.locfileid: "116783565"
---
# <a name="chapter-2---installation-and-use-of-azure-rtos-netx-duo-dhcpv6-server"></a>Capítulo 2 - Instalação e utilização do servidor Azure RTOS NetX Duo DHCPv6

Este capítulo contém uma descrição de vários problemas relacionados com a instalação, configuração e utilização do NetX Duo DHCPv6 Server.
...
**nxd_dhcpv6_server.h** Ficheiro de cabeçalho NetX DuoDHCPv6Server

**nxd_dhcpv6_server.c** NetX DuoDHCPv6Server ficheiro de origem

**demo_netxduo_dhcpv6.c** Ficheiro de demonstração do Servidor NetX Duo DHCPv6

**nxd_dhcpv6_server.pdf** Guia de utilizador NetX Duo DHCPv6Server

## <a name="netx-duo-dhcpv6-server-installation"></a>Instalação do servidor NetX Duo DHCPv6

Para utilizar o NetX Duo DHCPv6Server API, toda a distribuição mencionada anteriormente deve ser copiada para o mesmo diretório onde o NetX Duo está instalado. Por exemplo, se o NetX Duo for instalado no diretório "*\threadx\arm7\green*" então os ficheiros *nxd_dhcpv6_server.h* e *nx_dhpcv6_server.c* devem ser copiados para este diretório.

## <a name="using-netx-duo-dhcpv6-server"></a>Usando o servidor NetX Duo DHCPv6

A utilização da API NetX Duo DHCPv6Server é fácil. Basicamente, o código de aplicação deve incluir *nx_dhcpv6-server.h* depois de incluir *tx_api.h* e *nx_api.h,* para utilizar o ThreadX e o NetX Duo, respectivamente. O pedido também deve incluir *nxd_dhcpv6_server.c* no processo de construção. Este ficheiro deve ser compilado da mesma forma que outros ficheiros de aplicações e a sua forma de objeto deve ser ligada juntamente com os ficheiros da aplicação. Isto é tudo o que é necessário para usar o NetX Duo DHCPv6 Server.

Note que, uma vez que o DHCPv6is se baseia no protocolo IPv6, o IPv6 deve ser ativado na instância IP utilizando *nxd_ipv6_enable*. Os serviços NetX Duo UDP e ICMPv6 também são utilizados. O UDP é ativado através da chamada *nx_udp_enable* e ICMPv6is ativados por *nxd_icmp_enable* antes de iniciar a tarefa de thread do NetX Duo DHCPv6.

## <a name="small-example-system"></a>Sistema de Pequenos Exemplos

Um exemplo de como é fácil utilizar o NetX Duo DHCPv6 Server é descrito no pequeno exemplo abaixo usando um Cliente e Servidor DHCPv6 que passa por cima de um controlador virtual "RAM". Esta demonstração assume um único hospedeiro alojado usando o ambiente NetX Duo.

*tx_application_define* cria pacotes para envio de mensagem DHCPv6, um fio e uma instância IP tanto para o Cliente como para o Servidor, e permite que o UDP (DHCP passa sobre uDP), IPv6, ICMP e ICMPv6 para tarefas IP do Cliente e do Servidor nas linhas 116-157.

O Servidor DHCPv6 é criado na linha 456. Não define o declínio opcional do endereço ou os manipuladores de pedidos de opção. Na função de entrada de fio server, o IP do servidor é configurado com um link serviços de endereço local nas linhas 435-453.

Antes de iniciar o Servidor DHCPv6, a aplicação do anfitrião cria um Servidor DUID na linha 498 e define o servidor DNS da rede local na linha 483. Em seguida, cria uma tabela de endereços IP atribuíveis nas linhas 521. Consulte o **Sistema de Exemplos Avançados** no Apêndice D para saber como armazenar e recuperar as tabelas do Servidor da memória.

Em seguida, o Servidor DHCPv6 está pronto para iniciar a linha 530.

Para obter detalhes sobre a criação e execução do Cliente DHPPv6 da NetX Duo consulte o ficheiro *nxd_dhcpv6_client.pdf* distribuído com o Servidor DHCPv6.
...
**Figura 6. Exemplo do Servidor DHCPv6 da NetX Duo**