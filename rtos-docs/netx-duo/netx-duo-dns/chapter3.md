...
- **nx_dns_authority_zone_start_get** *Procure o início de uma zona de autoridade associada ao nome de anfitrião especificado*
...
- **nx_dns_cache_notify_set** *Desemalte a função de notificação completa da cache.*

- **nx_dns_cname_get** *Procure o nome de domínio canónico para o pseudónimo do nome de domínio de entrada*

- **nx_dns_create** *Criar uma instância de cliente DNS*
...
- **nx_dns_get_serverlist_size** *Devolver o tamanho da lista de servidores do Cliente DNS*

- **nx_dns_info_by_name_get** *Endereço IP de retorno, consulta de porta no nome do anfitrião de entrada*

- **nx_dns_ipv4_address_by_name_get** *Procure o endereço IPv4 a partir do nome de anfitrião especificado*

- **nxd_dns_ipv6_address_by_name_get** *Procure o endereço IPv6 a partir do nome de anfitrião especificado*

- **nx_dns_host_by_address_get** *função de invólucro para nxd_dns_host_by_address_get procurar um nome de anfitrião a partir de um endereço IP especificado (suporta apenas endereços IPv4)*

- **nxd_dns_host_by_address_get** *Procure um endereço IP a partir do nome do anfitrião de entrada (suporta os endereços IPv4 e IPv6)*

- **nx_dns_host_by_name_get** *função de invólucro para nxd_dns_host_by_address_get procurar um nome de anfitrião a partir do endereço especificado (suporta apenas endereços IPv4)*

- **nxd_dns_host_by_name_get** *Procure um endereço IP a partir do nome do anfitrião de entrada (suporta os endereços IPv4 e IPv6)*

- **nx_dns_host_text_get** *Procure os dados de texto para o nome de domínio de entrada*

- **nx_dns_packet_pool_set** *Definir a piscina de pacotes do cliente DNS*

- **nx_dns_server_add** *função de invólucro para* nxd_dns_server_add adicionar um Servidor *DNS no endereço especificado na lista de Clientes (suporta apenas o IPv4)*

- **nxd_dns_server_add** *Adicionar um Servidor DNS do endereço IP especificado à lista de servidores do Cliente (suporta os endereços IPv4 ou IPv6)*

- **nx_dns_server_get** *Devolver o Servidor DNS na lista de Clientes (suporta apenas endereços IPv4)*

- **nxd_dns_server_get** *Devolver o Servidor DNS na lista de Clientes (suporta os endereços IPv4 e IPv6)*

- **nx_dns_server_remove** *função de invólucro para nxd_dns_server_remove remover um Servidor DNS da lista de clientes*

- **nxd_dns_server_remove** *Remover um Servidor DNS do endereço IP especificado da lista de Clientes (suporta os endereços IPv4 e IPv6)*

- **nx_dns_server_remove_all** *Remover todos os Servidores DNS da lista de clientes*
...
No NetX Duo DNS Client, o tipo de registo SOA, NX_DNS_SOA_ENTRY, é guardado como sete parâmetros de byte 4, totalizando 28 bytes:

- **nx_dns_soa_host_mname_ptr** Ponteiro para a fonte primária de dados para esta zona.
...
- **nx_dns_soa_retry** Intervalo entre as recaírias da consulta soa
- **nx_dns_soa_expire** Duração do tempo quando o SOA expirar
- **nx_dns_soa_minmum** Campo TTL mínimo em palavras de resposta DO ESTADO
...
- **host_name** Ponteiro para o nome de anfitrião para obter dados soa para
...
- **NX_DNS_NO_SERVER** (0xA1) Lista de servidores de clientes está vazia
...
- **cache_size** Tamanho do DNS Cache, em bytes.

### <a name="return-values"></a>Valores de devolução

- **NX_SUCCESS** (0x00) DNS Cache iniciais com sucesso
- NX_DNS_PARAM_ERROR (0xA8) Entrada inválida sem ponteiro
- NX_DNS_CACHE_ERROR (0xB7) Ponteiro cache inválido.
- NX_PTR_ERROR (0x07) Ponteiro DNS inválido. 
- NX_DNS_ERROR cache (0xA0) não está alinhada com 4 bytes.
...
- **cache DNS** NX_SUCCESS (0x00) notificam com sucesso
...
- **record_buffer** Ponteiro para a localização para extrair dados do CNAME em
- **buffer_size** Tamanho do tampão para conter dados CNAME
- **wait_option** Aguarde a opção para receber a resposta do DNS Server

### <a name="return-values"></a>Valores de devolução

- **NX_SUCCESS** (0x00) obteve com sucesso dados do CNAME
- **NX_DNS_NO_SERVER** (0xA1) Lista de servidores de clientes está vazia
- **NX_DNS_QUERY_FAILED** (0xA3) Nenhuma resposta de DNS válida recebida
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS
- NX_CALLER_ERROR (0x11) Inválido deste serviço
...
> [!IMPORTANT]
> A aplicação deve garantir que a carga útil do pacote do pacote utilizado pelo Cliente DNS seja suficientemente grande para a mensagem máxima de 512 byte DNS, além dos cabeçalhos UDP, IP e Ethernet. Se o Cliente DNS criar a sua própria piscina de pacotes, isso é definido por NX_DNS_PACKET_PAYLOAD e NX_DNS_PACKET_POOL_SIZE.

Se a aplicação DO CLIENTE DNS preferir fornecer um conjunto de pacotes previamente criado, a carga útil para o IPv4 DNS Cliente deve ser de 512 bytes para o máximo DNS mais 20 bytes para o cabeçalho IP, 8 bytes para o cabeçalho UDP e 14 bytes para o cabeçalho Ethernet. Para o IPv6, a única diferença é que o cabeçalho IP é de 40 bytes, pelo que o pacote precisa acomodar o cabeçalho IPv6 de 40 bytes.

### <a name="input-parameters"></a>Parâmetros de Entrada

- **dns_ptr** Ponteiro para o Cliente DNS.  
- **ip_ptr** Ponteiro para instância IP previamente criada.  
- **domain_name** Ponteiro para nome de domínio para a instância DNS.

### <a name="return-values"></a>Valores de devolução

- **NX_SUCCESS** (0x00) DNS de sucesso criam
- **DNS NX_DNS_ERROR** (0xA0) criam erro
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS
- NX_CALLER_ERROR (0x11) Inválido deste serviço
...
- **NX_SUCCESS** (0x00) Cliente DNS bem sucedido.
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS.
- NX_CALLER_ERROR (0x11) Inválido deste serviço.
...
No Cliente DNS NetX Duo o tipo de dados NS, NX_DNS_NS_ENTRY, é guardado como dois parâmetros de 4 byte:

- **nx_dns_ns_ipv4_address** Endereço IPv4 do servidor de nome
...
- **buffer_size** Tamanho do tampão para conter dados NS
...
No NetX Duo DNS Client, o tipo de registo de troca de correio, NX_DNS_MAIL_EXCHANGE_ENTRY, é guardado como quatro parâmetros, totalizando 12 bytes:
...
No NetX Duo DNS Client, o tipo de registo de serviço, NX_DNS_SRV_ ENTRADA, é guardado como seis parâmetros, totalizando 16 bytes. Isto permite que os dados SRV de comprimento variável sejam armazenados de forma eficiente na memória:
...
- **record_buffer** Ponteiro para a localização para extrair dados DEV em
...
- **NX_SUCCESS** (0x00) obteve com sucesso dados de SRV
- **NX_DNS_NO_SERVER** (0xA1) Lista de servidores de clientes está vazia
- **NX_DNS_QUERY_FAILED** (0xA3) Nenhuma resposta de DNS válida recebida
- NX_DNS_PARAM_ERROR (0xA8) Parâmetro inválido sem ponte.
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS
- NX_CALLER_ERROR (0x11) Inválido deste serviço
...
- **NX_SUCCESS** (0x00) DNS Server tamanho da lista de servidor devolvido com sucesso
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS.
- NX_CALLER_ERROR (0x11) Inválido deste serviço
...
- **dns_ptr** Ponteiro para bloco de controlo DNS  
- **host_name** Ponteiro para o tampão de nome anfitrião
- **host_address_ptr** Ponteiro para endereçar para devolver
- **host_port_ptr** Ponteiro para o porto para regressar
- **wait_option** Opção de espera para a resposta dns
...
- **NX_DNS_QUERY_FAILED** (0xA3) consulta de DNS falhou; não há resposta de nenhum servidor DNS na lista de Clientes ou nenhum registo de serviço está disponível para o nome de anfitrião de entrada.
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS
- NX_CALLER_ERROR (0x11) Inválido
...
- **dns_ptr** Ponteiro para o Cliente DNS.  
- **host_name_ptr** Ponteiro para o nome de anfitrião para obter endereço IPv4
- **tampão** Ponteiro para a localização para extrair dados do IPv4 em
- **buffer_size** Tamanho do tampão para manter dados IPv4
- **wait_option** Aguarde a opção para receber a resposta do DNS Server
...
Procure o endereço IPv6 para o nome do anfitrião de entrada
...
O formato dos endereços IPv6 armazenados no tampão alinhado de 4 byte é mostrado abaixo:

![Tampão alinhado iPv6 4 byte](media/image9.png)

Se a entrada *record_buffer* não conseguir conter todos os dados AAAA na resposta do servidor, o *record_buffer* detém o número de registos que caberão e devolve o número de registos no tampão.

Com o número de registos AAAA devolvidos em **record_count,* a aplicação pode analisar os endereços IPv6 de cada registo no *record_buffer*.

### <a name="input-parameters"></a>Parâmetros de Entrada

- **dns_ptr** Ponteiro para o Cliente DNS.  
- **host_name_ptr** Ponteiro para o nome de anfitrião para obter endereço IPv6
- **tampão** Ponteiro para a localização para extrair dados do IPv6 em
- **buffer_size** Tamanho do tampão para manter dados IPv6
- **wait_option** Aguarde a opção para receber a resposta do DNS Server

### <a name="return-values"></a>Valores de devolução

- **NX_SUCCESS** (0x00) obteve com sucesso dados do IPv6
- **NX_DNS_NO_SERVER** (0xA1) Lista de servidores de clientes está vazia
- **NX_DNS_QUERY_FAILED** (0xA3) Nenhuma resposta de DNS válida recebida
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS
- NX_CALLER_ERROR (0x11) Inválido deste serviço
...
- **wait_option** Define quanto tempo o serviço irá esperar no temporizador para uma resposta do servidor DNS após cada consulta de DNS e redandição de consulta. As opções de espera são definidas da seguinte forma:

  valor de prazo (0x00000001-0xFFFFFFFE) TX_WAIT_FOREVER (0xFFFFFFFF)

  A seleção TX_WAIT_FOREVER faz com que o fio de chamada suspenda indefinidamente até que um servidor DNS responda ao pedido.

  A seleção de um valor numérico (1-0xFFFFFFFE) especifica o número máximo de temporizadores para permanecer suspenso enquanto se aguarda a resolução do DNS.

### <a name="return-values"></a>Valores de devolução

- **NX_SUCCESS** (0x00) Resolução de DNS bem sucedida  
- **NX_DNS_TIMEOUT** (0xA2) cronometrado na obtenção de mutex DNS
- **NX_DNS_NO_SERVER** (0xA1) Nenhum endereço dns server especificado
...
- **NX_DNS_PARAM_ERROR** (0xA8) Entrada inválida sem ponteiro
- **NX_DNS_IPV6_NOT_SUPPORTED** (0xB3) Não pode processar registo com IPv6 desativado
...
Procure um nome de anfitrião a partir do endereço IP
...
- **wait_option** Define quanto tempo o serviço irá esperar no temporizador para uma resposta do servidor DNS após cada consulta de DNS e redandição de consulta. As opções de espera são definidas da seguinte forma:
  ...
  valor de tempo limite (0x00000001 a 0xFFFFFFFE) TX_WAIT_FOREVER (0xFFFFFFFF)  
...
- **host_address_ptr** Endereço IP do DNS Server devolvido
- **wait_option** Define quanto tempo o serviço vai esperar pela resolução do DNS. As opções de espera são definidas da seguinte forma:
...
- **dns_ptr** Ponteiro para a instância do cliente DNS previamente criada.
- **host_name** Ponteiro para o nome de anfitrião para encontrar um endereço IP de
- **host_address_ptr** Ponteiro para destino para NXD_ADDRESS que contenha o endereço IP
- **wait_option** Define quanto tempo o serviço irá esperar nos tiques temporizadores para a resposta do DNS Server para cada transmissão e retransmissão de consultas. As opções de espera são definidas da seguinte forma:
...
- **lookup_type** Indicar tipo de procura (A vs AAAA).
...
Outro exemplo de utilização deste serviço de tempo, desta vez utilizando endereços IPv4 e tipos de registo A, é mostrado abaixo:
```C
...
> [!NOTE]
> O record_buffer não precisa de estar alinhado com 4 bytes para receber os dados.

### <a name="input-parameters"></a>Parâmetros de Entrada

- **dns_ptr** Ponteiro para o Cliente DNS.  
- **host_name** Ponteiro para o nome do anfitrião para pesquisar em
...
> [!NOTE]
> *W* hen o DNS Client é eliminado, o pacote de pacotes não é apagado com ele e é da responsabilidade da aplicação apagar o pacote de pacotes quando já não precisa dele.
>
> Este serviço só está disponível se a opção de configuração NX_DNS_CLIENT_USER_CREATE_PACKET_POOL for definida em *nxd_dns.h*
...
**NX_NO_MORE_ENTRIES** (0x17) Não são permitidos mais servidores DNS (a lista está completa)
- **NX_DNS_PARAM_ERROR** (0xA8) Entrada inválida sem ponteiro
- **NX_DNS_IPV6_NOT_SUPPORTED** (0xB3) Não pode processar registo com IPv6 desativado
- NX_PTR_ERROR (0x07) Entrada inválida do ponteiro
- NX_CALLER_ERROR (0x11) Inválido deste serviço
...
Adicionar servidor DNS à lista de clientes
...
- **dns_ptr** Ponteiro para o bloco de controlo DNS.
- **server_address** Ponteiro para o NXD_ADDRESS que contém o endereço IP do servidor DO DNS Server.
...
> [!NOTE]
> O índice é baseado em zero. Se o índice de entrada exceder o tamanho da lista de Clientes DNS, um endereço IPv6 é encontrado nesse índice ou um endereço nulo é encontrado no índice especificado, um erro é devolvido. O serviço *nx_dns_get_serverlist_size* pode ser chamado primeiro obter o número de servidores DNS na lista de Clientes.

Este serviço suporta apenas endereços IPv4. Chama ao serviço *nxd_dns_server_get* que suporta os endereços IPv4 e IPv6.

### <a name="input-parameters"></a>Parâmetros de Entrada

- **dns_ptr** Ponteiro para bloco de controlo DNS  
- **índice** Índice na lista de servidores do CLIENTE DNS
- **dns_server_address** Ponteiro para endereço IP do Servidor DNS

### <a name="return-values"></a>Valores de devolução

- **NX_SUCCESS** (0x00) Servidor de sucesso devolvido
- **Índice NX_DNS_SERVER_NOT_FOUND** (0xA9) aponta para slot vazio
- **índice NX_DNS_BAD_ADDRESS_ERROR** (0xA4) aponta para endereço nulo
- **NX_DNS_INVALID_ADDRESS_TYPE** (0xB2) O índice aponta para o tipo de endereço inválido (por exemplo, IPv6)
- **NX_DNS_PARAM_ERROR** (0xA8) Entrada inválida sem ponte
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS.
- NX_CALLER_ERROR (0x11) Inválido deste serviço
...
Devolva um Servidor DNS da lista de clientes
...
> [!NOTE]
> O índice é baseado em zero. Se o índice de entrada exceder o tamanho da lista de Clientes DNS, ou se um endereço nulo for encontrado no índice especificado, um erro é devolvido. O serviço *nx_dns_get_serverlist_size* pode ser chamado primeiro para obter o número de servidores DNS na lista de servidores.

Este serviço suporta endereços IPv4 e IPv6.
...
- **NX_SUCCESS** (0x00) endereço IP do servidor devolvido com sucesso
- **Índice NX_DNS_SERVER_NOT_FOUND** (0xA9) aponta para slot vazio
- **NX_DNS_BAD_ADDRESS_ERROR** (0xA4) Índice aponta para endereço de servidor nulo
- **NX_DNS_INVALID_ADDRESS_TYPE** (0xB2) O índice aponta para o tipo de endereço inválido (por exemplo, IPv6)
- **NX_DNS_PARAM_ERROR** (0xA8) Entrada inválida sem ponteiro
- NX_PTR_ERROR (0x07) Ponteiro IP inválido ou DNS.
- NX_CALLER_ERROR (0x11) Inválido deste serviço
...
Remova um Servidor DNS da lista de clientes
...
- **dns_ptr** Ponteiro para o bloco de controlo DNS.
- **server_address** O ponteiro para o SERVIDOR DNS NXD_ADDRESS dados que contenham o endereço IP do servidor.
