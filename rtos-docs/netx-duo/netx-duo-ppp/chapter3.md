...
ms.locfileid: "116798083"
---
# <a name="chapter-3---description-of-azure-rtos-netx-duo-point-to-point-protocol-ppp-services"></a>Capítulo 3 - Descrição dos serviços Azure RTOS NetX Duo Point-to-Point Protocol (PPP)

Este capítulo contém uma descrição de todos os serviços PPP Azure RTOS NetX Duo (listados abaixo) por ordem alfabética.
...
- **nx_ppp_dns_address_get**: *Obtenha o endereço IP do Servidor DNS*
...
Este serviço cria uma instância PPP para a instância IP netx especificada. Esta função deve ser chamada antes de criar a instância IP netX.
...
Obtenha o endereço IP do Servidor DNS
...
Este serviço recupera o endereço DNS IP fornecido pelo par no aperto de mão IPCP. Se nenhum endereço IP foi fornecido pelo par, um endereço IP de 0 é devolvido.

### <a name="input-parameters"></a>Parâmetros de Entrada

- **ppp_ptr**: Ponteiro para o bloco de controlo PPP.
- **dns_address_ptr**: Destino para endereço do servidor DNS
...
Este serviço configura os endereços IP locais e pares para utilização no Protocolo de Controlo do Protocolo de Internet (IPCP). Deve ser solicitado o caso PPP que tenha endereços IP válidos para si e para o outro par.
...
Este serviço envia um pedido de LCP e estabelece uma bandeira que o dispositivo PPP está à espera de uma resposta de eco. A opção de espera é principalmente para a *chamada nx_packet_allocate.* O serviço regressa assim que o pedido for enviado. Não espera uma resposta. 
...
Este serviço envia uma cadeia ASCII não PPP diretamente para fora da interface PPP. É normalmente usado após pPP receber um pacote não PPP que contém informações de controlo do modem.
