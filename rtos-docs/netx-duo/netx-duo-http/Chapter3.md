...
Este capítulo contém uma descrição de todos os serviços Azure RTOS NetX Duo HTTP (listados abaixo) por ordem alfabética, com exceção do equivalente 'NetX' (apenas IPv4) do mesmo serviço são emparelhados em conjunto).
...
- **nx_http_client_get_start** *Iniciar um pedido HTTP GET (apenas IPv4)*
- **nx_http_client_get_start_extended** *Iniciar um pedido HTTP GET (apenas IPv4)*
- **nxd_http_client_get_start** *Iniciar um pedido HTTP GET (IPv4 ou IPv6)*
- **nxd_http_client_get_start_extended** *Iniciar um pedido HTTP GET (IPv4 ou IPv6)*
- **nx_http_client_get_packet** *Obtenha o próximo pacote de dados de recursos*
- **nx_http_client_put_start** *Iniciar um pedido HTTP PUT (apenas IPv4)*
- **nx_http_client_put_start_extended** *Iniciar um pedido HTTP PUT (apenas IPv4)*
- **nxd_http_client_put_start** *Iniciar um pedido HTTP PUT (IPv4 ou IPv6)*
- **nxd_http_client_put_start_extended** *Iniciar um pedido HTTP PUT (IPv4 ou IPv6)*
...
- **nx_http_server_gmt_callback_set** *Definir chamada para recuperar data e hora gMT*
...
- **nx_http_server_param_get** *Obter o parâmetro do pedido*
...
- **nx_http_server_type_get (prevadado)** *Extrato HTTP tipo, por exemplo, texto/planície do cabeçalho*
...
- **nx_http_server_authentication_check_set** *Definir função de verificação de chamada de autenticação*
...
### <a name="input-parameters"></a>Parâmetros de Entrada
 ...
 - **pool_ptr** Ponteiro para piscina de pacote padrão. Note que os pacotes desta piscina devem ter uma carga útil suficientemente grande para manusear o cabeçalho de resposta completo. Isto é definido por NX_HTTP_CLIENT_MIN_PACKET_SIZE em *nx_http.h*.
...
### <a name="return-values"></a>Valores de devolução
 ...
 - NX_PTR_ERROR (0x07) Inválido HTTP, ip_ptr ou ponteiro de piscina de pacotes
...
### <a name="return-values"></a>Valores de devolução
 ...
 - NX_PTR_ERROR (0x07) Ponteiro HTTP inválido
...
Inicie um pedido HTTP GET sobre o IPv4
...
> [!NOTE]
> A cadeia de recursos pode referir-se a um ficheiro local, por ``` “/index.htm” ``` exemplo, ou pode referir-se a outro URL, por exemplo, ```http://abc.website.com/index.htm``` se o Servidor HTTP indicar que suporta pedidos GET de referência.

Este serviço funciona apenas através da rede IPv4. Para aplicações que precisam de se ligar à rede IPv6, *nxd_http_client_get_start_extended()* devem ser utilizadas.

Este serviço é precotado. Os desenvolvedores são encorajados a migrar para *nx_http_client_get_start_extended()* ou *nxd_http_client_get_start_extended()*.
...
### <a name="input-parameters"></a>Parâmetros de Entrada
 ...
 - **input_size** Número de bytes na entrada adicional opcional apontada por ```input_ptr``` .
 ...
 - **wait_option** Define quanto tempo o serviço irá esperar pelo pedido de início do cliente HTTP. As opções de espera são definidas da seguinte forma:
    ...
    A seleção TX_WAIT_FOREVER faz com que o fio de chamada suspenda indefinidamente até que o Servidor HTTP responda ao pedido.

    A seleção de um valor numérico (0x1-0xFFFFFFFE) especifica o número máximo de temporizadores para permanecer suspenso enquanto se aguarda a resposta do servidor HTTP.

### <a name="return-values"></a>Valores de devolução

 - **NX_SUCCESS** (0x00) enviou com sucesso o cliente HTTP. Get mensagem de início.
...
### <a name="input-parameters"></a>Parâmetros de Entrada
 ...
 - **client_ptr** Ponteiro para http bloco de controlo do cliente.
 - **ip_address** Endereço IP do servidor HTTP.
 - **Recurso Pointer** para url string para recurso solicitado.
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **NX_SUCCESS** (0x00) enviou com sucesso o cliente HTTP. Mensagem de início GET
 - **NX_HTTP_ERROR** (0xE0) Erro interno do cliente HTTP
 - **NX_HTTP_NOT_READY** (0xEA) HTTP Cliente não está pronto
 - **NX_HTTP_FAILED** (0xE2) HTTP Erro do cliente que comunica com o servidor HTTP.
 - **NX_HTTP_AUTHENTICATION_ERROR** (0xEB) Nome inválido e/ou senha.
...
Envie um pedido HTTP GET (IPv4 ou IPv6)
...
Este serviço é precotado. Os desenvolvedores são encorajados a migrar para *nxd_http_client_get_start_extended()*.

### <a name="input-parameters"></a>Parâmetros de Entrada

 - **client_ptr** Ponteiro para http bloco de controlo do cliente.
 - **Server_ip** Endereço IP do servidor HTTP.
 - **recurso** Ponteiro para cadeia URL para recurso solicitado.
 - **input_ptr** Ponteiro para dados adicionais para o pedido GET. Isto é opcional. Se válido, a entrada especificada é colocada na área de conteúdo da mensagem e um POST é utilizado em vez de uma operação GET.
 - **input_size** Número de bytes na entrada adicional opcional apontada por ```input_ptr``` .
 - **nome de utilizador** Ponteiro para o nome de utilizador opcional para a autenticação.
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **NX_SUCCESS** (0x00) enviou com sucesso pedido GET
 - **NX_HTTP_PASSWORD_TOO_LONG** (0xF0) Palavra-passe excede o tamanho do tampão
 - **NX_HTTP_NOT_READY** (0xEA) HTTP Cliente não está pronto
 - **NX_HTTP_FAILED** (0xE2) Parâmetros de pacote inválidos.
 - **NX_HTTP_AUTHENTICATION_ERROR** (0xEB) Nome ou senha inválida
 - NX_PTR_ERROR (0x07) Entrada inválida do ponteiro
 - NX_CALLER_ERROR (0x11) Inválido deste serviço
...
Este serviço substitui *nxd_http_client_get_start()*. Requer que o chamador especifique o comprimento do recurso, nome de utilizador e senha.
...
### <a name="input-parameters"></a>Parâmetros de Entrada
 ...
 - **senha** Ponteiro para senha opcional para autenticação.
 - **password_length** Duração da senha opcional para autenticação.
 - **wait_option** Define quanto tempo o serviço esperará internamente para processar o cliente HTTP começar. As opções de espera são definidas da seguinte forma:
    - **valor de saída de tempo** (0x00000001 através de 0xFFFFFFFE)
    - **TX_WAIT_FOREVER** (0xFFFFFFFF)
...
Inicie um pedido HTTP PUT sobre o IPv4
...
> [!NOTE]
> A cadeia de recursos pode referir-se a um ficheiro local, por ``` “/index.htm” ``` exemplo, ou pode referir-se a outro URL, por exemplo, ```http://abc.website.com/index.htm``` se o servidor HTTP indicar que suporta pedidos DE REenso.

Este serviço é precotado. Os desenvolvedores são encorajados a migrar para *nxd_http_client_put_start_extended()*.
...
Inicie um pedido HTTP PUT (IPv4 ou IPv6)
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **NX_SUCCESS** (0x00) enviaram com sucesso o pedido HTTP Client PUT
 - **NX_HTTP_ERROR** (0xE0) HTTP Erro interno do cliente
 - **NX_HTTP_NOT_READY** (0xEA) HTTP Cliente não está pronto
 - **NX_HTTP_FAILED** (0xE2) HTTP Erro do cliente que comunica com o servidor HTTP
 - NX_PTR_ERROR (0x07) Entrada inválida do ponteiro
 - NX_SIZE_ERROR (0x09) Tamanho total inválido de recursos
...
Este serviço substitui *nxd_http_client_put_start()*. Requer que o chamador especifique o comprimento do recurso, nome de utilizador e senha.
...
> [!NOTE]
> Esta rotina deve ser chamada repetidamente até que o comprimento combinado dos pacotes enviados seja igual ao "total_bytes" especificado na chamada *nx_http_client_put_start* anterior.
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **NX_HTTP_NOT_READY** (0xEA) HTTP Cliente não está pronto
 - **NX_HTTP_REQUEST_UNSUCCESSFUL_CODE** (0xEE) Código de erro do Servidor Recebido
 - **NX_HTTP_BAD_PACKET_LENGTH** (0xED) Comprimento do pacote inválido
 - **NX_HTTP_AUTHENTICATION_ERROR** (0xEB) Nome inválido e/ou senha
...
Definir a porta de ligação para o Servidor
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **NX_SUCCESS** (0x00) mude com sucesso a porta
...
> [!NOTE]
> Se esta função for utilizada, a rotina de retorno é responsável pelo envio de toda a resposta no formato adequado. Além disso, a rotina de retorno deve devolver o estado de NX_HTTP_CALLBACK_COMPLETED.
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **NX_SUCCESS** (0x00) enviou com sucesso o pacote do Servidor
 - NX_PTR_ERROR (0x07) Entrada inválida do ponteiro
...
> [!NOTE]
> Se esta função for utilizada, a rotina de retorno deve devolver o estado de NX_HTTP_CALLBACK_COMPLETED.

Este serviço é precotado. Os desenvolvedores são encorajados a migrar para *nxd_http_server_callback_response_send_extended()*.
...
Este serviço substitui *nx_http_server_callback_response_send()*. Esta versão requer informações adicionais de comprimento como argumentos.
...
Obtenha a duração do conteúdo no pedido
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **NX_SUCCESS** (0x00) Conteúdo satisfatório do servidor obtém
...
> [!NOTE]
> NX_HTTP_MULTIPART_ENABLE deve ser habilitado a utilizar este serviço.

Consulte [*nx_http_server_get_entity_header*](#nx_http_server_get_entity_header) para mais detalhes.
...
### <a name="input-parameters"></a>Parâmetros de Entrada
 ...
 - **server_ptr** Ponteiro para o servidor HTTP
 - **gmt_getv** Ponteiro para retorno GMT
 - **datev** Ponteiro para a data recuperada
...
### <a name="input-parameters"></a>Parâmetros de Entrada
 ...
 - **client_address** Ponteiro para endereço do cliente. Pode ser IPv4 ou IPv6
...
Se a função de notificação de pedido estiver registada no servidor HTTP, o pedido de notificação de chamada pode ligar *nx_http_server_type_retrieve()* para analisar o tipo de ficheiro.
...
> [!NOTE]
> Isto não deve ser chamado antes de chamar *nx_http_server_get_entity_header* porque modifica o ponteiro pré-final para além do cabeçalho da entidade.
...
### <a name="input-parameters"></a>Parâmetros de Entrada
 ...
 - **content_length** Ponteiro para extrair ```content_length```
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **NX_SUCCESS** (0x00) recebeu com sucesso o próximo pacote
 - **NX_HTTP_TIMEOUT** (0xE1) O tempo expirou à espera do próximo pacote
 - NX_PTR_ERROR (0x07) Entrada inválida do ponteiro
...
### <a name="return-values"></a>Valores de devolução
 ...
 - **Comprimento da corda em bytes** Não tem valor zero é sucesso. Zero indica erro.
...
### <a name="input-parameters"></a>Parâmetros de Entrada
 ...
 - **http_type_string** (Ponteiro para o tipo HTML extraído)
 - **http_type_string_max_size** Tamanho do tampão http_type_string
...
Para um exemplo mais detalhado, consulte a descrição [*para nx_http_server_callback_generate_response_header*](#nx_http_server_callback_generate_response_header).
...
Definir função de verificação de autenticação
